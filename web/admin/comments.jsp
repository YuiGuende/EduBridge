...
<%@ page import="com.example.dto.CommentListDTO" %>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
                          ...
                          <div class="row g-3">
                              <div class="col-md-4">
                                  ...
                                  <input type="text" class="form-control" id="searchCourse" name="course" 
                                         value="<%= JSPUtils.htmlEncode(request.getParameter("course")) %>" 
                              ...
                              <div class="col-md-4">
                                  ...
                                  <input type="text" class="form-control" id="searchUser" name="user" 
                                         value="<%= JSPUtils.htmlEncode(request.getParameter("user")) %>" 
                          ...
                          <table class="table table-hover">
                              ...
                              <tbody>
                                  <c:forEach var="comment" items="${comments}">
                                      <tr>
                                          <td>${JSPUtils.htmlEncode(comment.id)}</td>
                                          <td>
                                              <div class="fw-bold">${JSPUtils.htmlEncode(comment.user.fullname)}</div>
                                              <div class="text-muted small">${JSPUtils.htmlEncode(comment.user.email)}</div>
                                          </td>
                                          <td>
                                              <div class="fw-bold">${JSPUtils.htmlEncode(comment.course.title)}</div>
                                              <div class="text-muted small">ID: ${JSPUtils.htmlEncode(comment.course.id)}</div>
                                          </td>
                                          <td>
                                              <div class="comment-content" style="max-width: 300px;">
                                                  <c:choose>
                                                      <c:when test="${comment.content.length() > 50}">
                                                          <span class="comment-preview">${JSPUtils.htmlEncode(comment.content.substring(0, 50))}...</span>
                                                          <span class="comment-full d-none">${JSPUtils.htmlEncode(comment.content)}</span>
                                                          <br>
                                                      ...
                                                      <c:otherwise>
                                                          ${JSPUtils.htmlEncode(comment.content)}
                                                      </c:otherwise>
                                                  </c:choose>
                                              </div>
                                          </td>
                                          ...
                                          <td>
                                              <div class="btn-group" role="group">
                                                  <a href="${pageContext.request.contextPath}/admin/comments?action=details&id=${JSPUtils.htmlEncode(comment.id)}" class="btn btn-sm btn-info">View Full Comment</a>
                                                  <a href="${pageContext.request.contextPath}/admin/comments?action=toggleStatus&id=${JSPUtils.htmlEncode(comment.id)}" class="btn btn-sm btn-warning">Toggle Status</a>
                                                  <a href="${pageContext.request.contextPath}/admin/comments?action=delete&id=${JSPUtils.htmlEncode(comment.id)}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this comment?');">Delete</a>
                                              </div>
                                          </td>
                                      </tr>
                                  </c:forEach>
                          ...
                          <nav aria-label="Comment pagination">
                              <ul class="pagination justify-content-center">
                                  <c:if test="${currentPage > 1}">
                                      <li class="page-item">
                                          <a class="page-link" href="<%= JSPUtils.buildPaginationUrl(request, (Integer)request.getAttribute("currentPage") - 1) %>">
                                              Previous
                                          </a>
                                      </li>
                                  </c:if>
                                  ...
                                  <c:forEach begin="1" end="${totalPages}" var="i">
                                      <li class="page-item ${i == currentPage ? 'active' : ''}">
                                          <a class="page-link" href="<%= JSPUtils.buildPaginationUrl(request, (Integer)pageContext.getAttribute("i")) %>">
                                              ${i}
                                          </a>
                                      </li>
                                  </c:forEach>
